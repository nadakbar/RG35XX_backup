...
function listDirectories() {
    return new Promise((resolve, reject) => {
        const command = `${adbPath} -d shell "ls -d /mnt/mmc/*"`;
...
function adbPull(sourceDir) {
    return new Promise((resolve, reject) => {
        ...
        console.log('\x1b[33m', 'This could take a while and will NOT provide progress, please wait until Completion message', '\x1b[0m');
...
function createZip(directory) {
    return new Promise((resolve, reject) => {
        ...
        output.on('close', () => {
            console.log('\x1b[32m', `Backup directory "${directory}" has been zipped successfully.`, '\x1b[0m');
            resolve();
        });
        ...
        archive.on('progress', ({ entries }) => {
            ...
            console.clear();
            console.log('\x1b[32m', `Zipping "${directory}" Please wait...`, '\x1b[0m');
            console.log("Processed:", entries.processed, "Total Files:", entries.total, "Percentage Complete: ", percentage.toFixed(2), "%");
        });
...
async function run() {
    try {
        ...
        const selectedDirectoryIndex = parseInt(await prompt('Enter the index of the source directory: '), 10) - 1;

        if (isNaN(selectedDirectoryIndex) || selectedDirectoryIndex < 0 || selectedDirectoryIndex >= directories.length) {
        ...
        console.log('\x1b[32m', 'Backup Complete, Please see your backup directory', '\x1b[0m');

        // Ask if zipping is required
        const zipRequired = await prompt('Do you want to zip the backup directory? (Y/N): ');
